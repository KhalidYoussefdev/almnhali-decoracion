        ...
        <div className="lg:col-span-2 space-y-4">
          {cartProducts.map(({ product, quantity, productId }, i) => (
            <motion.div
              ...
              className="flex gap-4 p-4 bg-white dark:bg-navy-800 rounded-2xl shadow-sm"
            >
              <div className="relative w-24 h-24 rounded-xl overflow-hidden flex-shrink-0">
                <Image src={product.images[0]} alt="" fill className="object-cover" sizes="96px" />
              </div>
              <div className="flex-1 min-w-0">
                <h3 className="font-display text-lg text-navy dark:text-cream truncate">
                  {getLocalizedField(product, 'name', locale)}
                </h3>
                <p className="text-gold font-semibold mt-1">{formatPrice(product.price, locale)}</p>
                <div className="flex items-center gap-3 mt-3">
                  <button onClick={() => updateQuantity(productId, quantity - 1)} className="p-1 rounded-md hover:bg-beige">
                  ...
                  <button onClick={() => removeItem(productId)} className="p-1 ms-auto text-terracotta hover:bg-terracotta/10 rounded-md">
        ...
        <div className="lg:col-span-1">
          <div className="sticky top-28 p-6 bg-white dark:bg-navy-800 rounded-2xl shadow-md space-y-4">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="font-medium">{shipping === 0 ? 'Free' : formatPrice(shipping, locale)}</span>
            </div>
            {subtotal < 500 && (
              <p className="text-xs text-gold">{t('freeShipping')}</p>
            )}
            <div className="border-t border-beige-dark/30 pt-4 flex justify-between font-display text-xl">
              <span>{t('total')}</span>
              <span>{formatPrice(total, locale)}</span>
            </div>
            <Link href="/checkout" className="block">
              <Button variant="gold" size="lg" className="w-full">{t('checkout')}</Button>
            </Link>
          </div>
        </div>
      </div>
